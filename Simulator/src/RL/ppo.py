...
def transform_buffer(buff):
    s_batch, odom_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], [], []
    s_temp, odom_temp = [], []
    ...
    for e in buff:
        for state in e[0]:
            ...
            odom_temp.append(state[1])

        s_batch.append(s_temp)
        odom_batch.append(odom_temp)
    ...
    odom_batch = np.asarray(odom_batch)
    ...
    return s_batch, odom_batch, a_batch, r_batch, d_batch, l_batch, v_batch


def generate_action(env, input_list, policy, action_bound):
    if env.index == 0:
        depth_list, odom_list = [], []
        for i in input_list:
            depth_list.append(i[0])
            odom_list.append(i[1])


        depth_list = np.asarray(depth_list)
        odom_list = np.asarray(odom_list)

        depth_list = Variable(torch.from_numpy(depth_list)).float().cuda()
        odom_list = Variable(torch.from_numpy(odom_list)).float().cuda()


        v, a, logprob, _ = policy(depth_list, odom_list)
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
               ...
               num_step=2048, num_env=12, frames=1, obs_size=(640,480), act_size=4):
    obss, odoms, actions, logprobs, targets, values, rewards, advs = memory

    advs = (advs - advs.mean()) / advs.std()

    obss = obss.reshape((num_step*num_env, frames, obs_size[0],obs_size[1]))
    odoms = odoms.reshape((num_step*num_env, 6))
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            sampled_odoms = Variable(torch.from_numpy(odoms[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_odoms, sampled_actions)
