...
public:
    SensorSimulator(ros::NodeHandle &nh) : nh_(nh) {
        ...
        std::string collision_topic = config["collision_topic"].as<std::string>();
        ...
        odom_sub_ = nh_.subscribe(odom_topic, 10, &SensorSimulator::odomCallback, this);
    ...
    bool collision_detection(const pcl::PointCloud<pcl::PointXYZ> &cloud);
...
private:
    ...
    ros::NodeHandle nh_;
    // ros::Publisher image_pub_, point_cloud_pub_, collision_pub_;
    ros::Publisher image_pub_, collision_pub_;
...
bool SensorSimulator::collision_detection(const pcl::PointCloud<pcl::PointXYZ> &cloud)
{
    // ========== 高效碰撞检测 ==========
    const float safety_distance_ = 0.3f;
    const float safety_width_ = 0.3f;
    const float safety_height_ = 0.3f;
    const int min_points_threshold_ = 3;
    // if (cloud == NULL) {
    //     return;
    // }
    ...
    for (const auto& point : cloud.points) {
        // 检查是否在安全区域内
        if (point.x > 0 && 
            point.x < safety_distance_ &&
            std::abs(point.y) < safety_width_ / 2.0 &&
            std::abs(point.z) < safety_height_ / 2.0) {
            danger_count++;
        }
    }

    // 判断是否触发碰撞警告
    return (danger_count >= min_points_threshold_);
}
...
//     collision_msg.data = collision_detection(lidar_points);
